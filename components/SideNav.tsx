...
/**
 * Persistent left rail: the brand at a fixed top (never scrolls away, and
 * collapses to a slim rail on request), then task-first navigation with
 * practice area as an optional secondary narrow. Practice-specific
 * subcategories only appear once a practice is chosen, so the primary list
 * stays short.
 */
export function SideNav({
  ...
  return (
    <>
      ...
      <nav
        className={["side-nav", isOpen && "open", collapsed && "collapsed"].filter(Boolean).join(" ")}
        aria-label="Browse by task and practice area"
      >
        <div className="side-nav-topbar">
          ...
          <div className="side-nav-topbar-actions">
            <button
